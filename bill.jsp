...
try {
    ...
    PreparedStatement pst = con.prepareStatement("SELECT * FROM users INNER JOIN cart ON users.email = cart.email WHERE cart.email=? AND cart.status='bill' AND cart.product_id = (SELECT MAX(product_id) FROM cart WHERE email=? AND status='bill')");
